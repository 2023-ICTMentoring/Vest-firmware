...
void TMR0_IRQHandler(void){ // Timer0, 10ms
	...
	if(isflag(F_OVERTIME_CHECK)){
		...
		if(run.student_overtime_ms == 301){
			setflag(F_TIME_WARN);
...
void run_game_mode(void){
	switch(run.gamemode_sw){
		...
		case 51:
			if(isflag(F_RUN_FONT_ACTION)){
				random_chosung_font_Display(run.student_font);
				clrflag(F_RUN_FONT_ACTION);				
			}	
			break;
...
void random_chosung_font_Display(unsigned char chosung){    
    switch (chosung) {
		case 0:        A_Display();      break;//	ㄱ
		case 1:        B_Display();      break;//	ㄴ
		case 2:        C_Display();      break;//	ㄷ
		case 3:        D_Display();      break;//	ㄹ
		case 4:        E_Display();      break;//	ㅁ
		case 5:        F_Display();      break;//	ㅂ
		case 6:        G_Display();      break;//	ㅅ
		case 7:        H_Display();      break;//	ㅇ
		case 8:        J_Display();      break;//	ㅈ
		case 9:        K_Display();      break;//	ㅊ
		case 10:       L_Display();      break;//	ㅋ
		case 11:       M_Display();      break;//	ㅍ
		case 12:       N_Display();      break;//	ㅌ
		case 13:      OP_Display();      break;//   ㅎ	
    }
}
