			...
			while(run.while_event){	
				...
				if(cpass == '#'){ // exit
					...
					if(cpass == '*'){ // key back
						...
						// run.student_chosung_font[number];
					}
				}
...
void run_function(unsigned char func){
	...
	switch(func){	
		...
		case FUC_CHOSUNG_FONT : // 0x77		
			i = run.student_chosung_font[0];
			j = run.student_chosung_font[1];
			for(number = 0; number < run.howmanystudent; number++){
				run.senddata[number] = run.student_chosung_font[number];
				run_student_send(run.student_attend[number], CMD_CHOSUNG_FONT, 1); // 1 byte font each address game font
				time0_delay(TM_200mS);
			}
